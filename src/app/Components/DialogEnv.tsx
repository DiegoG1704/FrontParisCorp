...
export default function DialogRemitente({ Close, Open,Datos}: Props) {
...
console.log(Datos);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axiosInstance.post(`PostNotificaciones/${Datos?.remitente}`, {
        mensaje: respuesta,
        titulo: titulo,
        userId: user?.id
      })
      ...
      <Dialog header={Titulo} onHide={Close} visible={Open} className="w-[25rem]">
        <div className="flex flex-col gap-3">
          <div className='bg-green-300 p-4 rounded-md'>
            <span className='font-semibold'>Responder a {Datos?.nombres}</span> <br/>
            <span className='font-semibold'>Mensaje: </span><span>{Datos?.mensaje}</span>
          </div>
