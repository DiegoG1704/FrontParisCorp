...
export default function DialogEstado1({ Close, Open, Datos }: Props) {
  const { ListaPedidos } = useAppContext();
  const toast = useRef<Toast>(null);

  const handleSudmit = async () => {
    try {
      await axiosInstance.put(`/CambiarEstadoPedido/${Datos.id}/2`);
      ListaPedidos();

      toast.current?.show({
        severity: 'success',
        summary: 'Estado actualizado',
        detail: 'El pedido ahora está en camino',
        life: 3000
      });
      ...
      >
        <div className="p-4 space-y-6">
          <p className="text-gray-700 text-lg font-medium">
            ¿Confirmas que los productos del pedido del cliente{" "}
            <span className="font-bold text-[#BACD00]">{Datos.nombre}</span> salieron del almacén?
          </p>
