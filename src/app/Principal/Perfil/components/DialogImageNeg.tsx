...
export default function DialogImageNegocio({ Open, Close }: Props) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    formData.append("perfilNegocio", imagen);

    const response = await axiosInstance.put(
      `CambioFotoNegocio/${user?.id}`,
      formData,
      {
        headers: {
    ...
    setPreview(`/uploads/${response.data.imagenTaller}`);
    toast.current?.show({
      severity: "success",
      summary: "Listo",
